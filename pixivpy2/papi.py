...
class Pixiv_PAPI(object):
	...
	def get_image(self,url):
		...
		headers = {
			'Authorization': 'Bearer %s' % self.api.access_token,
			'Cookie': 'PHPSESSID=%s' % self.api.session,
			#'Referer' : 'http://www.pixiv.net/member_illust.php?mode=medium&illust_id=%s' % id
		}
		params = {
			'stream' : True
		}

		r = self.api._requests_call('IMAGE', url, headers=headers, params=params)
		return r.raw
	...
	def works(self, illust_id):
		...
		url = 'https://public-api.secure.pixiv.net/v1/works/%s.json' % (illust_id)
		headers = {
			'Authorization': 'Bearer %s' % self.api.access_token,
			'Cookie': 'PHPSESSID=%s' % self.api.session,
		}
	...
	def ranking_all(self, mode='daily', page=1, per_page=30, date=None,
